...
export async function POST(_req: NextRequest) {
  ...
  session.oauth.payer = { codeVerifier, state };
  // Persist payer config here as well to avoid missing cfg in callback if prior step failed
  session.payerLogin = { baseUrl, clientId, callbackUrl: redirectUri };
  await session.save();
