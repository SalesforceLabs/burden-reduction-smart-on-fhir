...
export default function ProviderLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/config/update', {
      ...
      body: JSON.stringify({ userType: 'providerLogin', baseUrl, clientId, callbackUrl: redirectUri })
    });
    if (!res.ok) return;
    const authStart = await fetch('/api/oauth/provider/start', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    <div className="center-div" id="baseLogin">
      <div className="container w-25 border py-5">
        <div className="title pb-2">
          <h2 className="heading-style">Provider Login</h2>
          <span>Log in to the Provider Org</span>
          <hr />
        </div>
